...
Both dancers are 50 × 16 px torso capsules. The narrow front-to-back depth allows close in-and-out passes while the rounded shoulder ends avoid corner catches. The follow always faces the leader; the leader rotates toward the follow at a capped 4 radians per second, which gives its orientation continuity without adding a second control gesture. Visuals and collision use the same capsule geometry. These defaults reflect iterative playtesting; document the reason when changing them.
